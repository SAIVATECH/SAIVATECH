        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-32">
          ...
          >
            ...
            <p className="text-slate-400 leading-relaxed text-lg">
              Our mission is not to build templates. Our mission is to deploy automated ecosystems that actively lower customer acquisition costs while multiplying conversion rates through AI logic and high-end aesthetics.
            </p>
          </motion.div>
            ...
            {teamData.map((member, idx) => (
              ...
              >
                ...
                <div className="p-8 relative z-20">
                  <h4 className="text-4xl md:text-5xl font-bold text-white mb-1 group-hover:text-[#00FFFF] transition-colors">{member.name}</h4>
                  <span className="text-[#00b8cc] font-mono text-lg uppercase tracking-widest block mb-4">{member.role}</span>
                  <p className="text-slate-400 text-base md:text-lg leading-relaxed">{member.bio}</p>
                </div>
              </motion.div>
            ))}
          </div>
